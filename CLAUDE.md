...
- **Merge the `discord-bot` branch** — Interactive Discord bot (slash commands incl. natural-language `/ask`) is fully implemented on the local `discord-bot` branch (commit `946af3c`) but not merged to main. Plan: `plans/discord-bot.md`. Note: `npm start` on main already references `src/discord-bot.ts`, which only exists on that branch — `npm start`'s bot process fails until merged.
